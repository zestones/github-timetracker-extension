...
export function RepoDetailView({ repo, repoDetails, userMode, onBack }) {
    ...
    const sorted = useMemo(() => {
        ...
        list.sort((a, b) => {
            let av, bv;
            if (key === 'time') { av = a.totalSeconds; bv = b.totalSeconds; }
            else if (key === 'sessions') { av = a.sessionCount; bv = b.sessionCount; }
            else if (key === 'avg') { av = a.avgSession; bv = b.avgSession; }
            else if (key === 'days') { av = a.uniqueDays; bv = b.uniqueDays; }
            else { av = a.totalSeconds; bv = b.totalSeconds; }
        ...
        <div className="p-4">
            ...
            <div className="grid grid-cols-4 gap-2 mb-4">
                {stats.map((s) => (
                    <div key={s.label} className="bg-surface rounded-lg p-2 text-center">
                        ...
                        <div className="text-[13px] font-bold text-primary font-mono tabular-nums mt-0.5">{s.value}</div>
                    </div>
                ))}
            </div>
            ...
            {userMode === 'everyone' && allUsers.length > 1 && (
                <div className="flex items-center gap-1.5 mb-3 flex-wrap">
                    ...
                    {allUsers.map(u => (
                        <span key={u} className="text-[10px] text-secondary bg-raised px-1.5 py-0.5 rounded-full">{u}</span>
                    ))}
                </div>
            )}
            ...
            <div className="space-y-1">
                {sorted.length === 0 ? (
                    <div className="text-[12px] text-tertiary text-center py-6">
                    ...
                    return (
                        <div key={issue.url}>
                            <div
                                className="flex items-start gap-1.5 py-2 px-2 rounded-lg hover:bg-surface cursor-pointer transition-colors"
                                ...
                                <div className="flex-1 min-w-0">
                                    <div className="flex items-baseline justify-between gap-2">
                                        <span className="text-[12px] text-primary truncate">
                                            {issue.issueNumber && (
                                                <span className="text-tertiary font-mono mr-1">{issue.issueNumber}</span>
                                            )}
                                            {issue.title}
                                        </span>
                                    ...
                                    <div className="flex items-center gap-3 mt-0.5 text-[10px] text-tertiary">
                                        <span>{issue.sessionCount} session{issue.sessionCount !== 1 ? 's' : ''}</span>
                                        <span>avg {TimeService.formatTime(issue.avgSession)}</span>
                                        <span>{issue.uniqueDays} day{issue.uniqueDays !== 1 ? 's' : ''}</span>
                                    </div>
                                </div>
                            </div>
                            {isOpen && (
