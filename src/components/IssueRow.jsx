...
export function IssueRow({ issue, isActive, onStart, onStop, trackedSeconds = 0 }) {
    ...
    return (
        <div
            className={`flex items-center gap-2 py-2 px-2 hover:bg-surface rounded-lg group transition-colors ${isClosed ? 'opacity-70' : ''}`}
        >
            <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-2 text-[11px] text-muted mt-0.5">
                    {trackedSeconds > 0 && <span className="font-mono">{TimeService.formatTime(trackedSeconds)}</span>}
                    <a
